...
class TubeFlowAPI:
    ...
    def _download_worker(self, url_or_urls, format_type, start_time, end_time):
        try:
            ...
            for idx, url in enumerate(urls):
                ...
                def progress_hook(d):
                    ...
                    if d['status'] == 'downloading':
                        ...
                        self.window.evaluate_js(
                            f"window.updateProgress(100, '', 'Processing...', {current_idx}, {total_items})"
                        )
                ...
                def postprocessor_hook(d):
                    ...
                    friendly_name = pp_name
                    if pp_name == 'ExtractAudio':
                        friendly_name = "Extracting audio track..."
                    elif pp_name == 'Merger':
                        friendly_name = "Merging video and audio streams..."
                    elif pp_name == 'EmbedThumbnail':
                        friendly_name = "Embedding thumbnail artwork..."
                    elif pp_name == 'MoveFiles':
                        friendly_name = "Saving file to destination..."
                    elif pp_name == 'VideoRemuxer':
                        friendly_name = "Remuxing video..."
                    elif pp_name == 'VideoConvertor':
                        friendly_name = "Converting video format..."
                    elif pp_name:
                        friendly_name = f"Processing ({pp_name})..."
                    else:
                        friendly_name = "Processing files..."
                    
                    if status == 'started':
                        status_title = f"Processing item {current_idx} of {total_items}" if total_items > 1 else "Processing..."
                        self.window.evaluate_js(
                            f"window.updatePostProcessing({json.dumps(status_title)}, {json.dumps(friendly_name)})"
                        )
            ...
            import traceback
            traceback.print_exc()
            self.window.evaluate_js(f"window.downloadFailed({json.dumps('Unexpected error: ' + str(e))})")
